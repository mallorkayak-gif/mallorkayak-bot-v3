...
def parse_openweather(data, day_idx):
    ...
    # Si no hay suficientes datos para ese día, usar los que haya
    if start_idx >= len(data["list"]):
        return None  # El día está completamente fuera de rango
    
    # Limitar end_idx al máximo disponible
    end_idx = min(end_idx, len(data["list"]))
    
    day_data = data["list"][start_idx:end_idx]
    
    if not day_data:  # Si la lista está vacía
        return None
    
    winds = [item["wind"]["speed"] for item in day_data]  # m/s
    temps = [item["main"]["temp"] for item in day_data]
    
    if not winds or not temps:  # Si no hay datos válidos
        return None
...
def parse_weatherapi(data, day_idx):
    ...
    # Si el día solicitado está fuera de rango, retornar None
...
def calculate_day_average(zona_name, lat, lon, day_idx):
    ...
    # Si no hay datos de NINGUNA API, retornar None (descartada)
    # PERO si hay al menos 1, mantenerla aunque falten 2 APIs
    if len(valid_data) == 0:
        return None
...
def generate_report():
    ...
    # Debug: mostrar cuántas zonas se recuperaron
    print("\n📊 RESUMEN POR DÍA:")
    for day_idx in dias:
        count = len(dias[day_idx]["zonas"])
        print(f"  {dias[day_idx]['nombre_dia']}: {count}/15 zonas")
    print()
    ...
    for day_idx in range(3):
        ...
        if day_info["zonas"]:
            # Mostrar TODAS las zonas (sin filtro)
